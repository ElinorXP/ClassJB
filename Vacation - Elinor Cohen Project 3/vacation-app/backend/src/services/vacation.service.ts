...
export class VacationService{
    async getVacations(user : IUser, followedMode: boolean, page: number = 0, rowsPerPage: number = 10): Promise<IVacationsData>{
        ...
        const results = await pool.promise().query(`SELECT *, 
            ...
            ORDER BY startDate DESC LIMIT ${rowsPerPage} OFFSET ${page * rowsPerPage}`);
